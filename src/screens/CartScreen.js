...
export default function CartScreen({ navigation }) {
  ...
  const [confirmType, setConfirmType] = useState(null); // 'checkout' | 'clear'
  ...
  const updateQuantity = (id, newQty) => {
    ...
    db.runSync("UPDATE cart_items SET quantity = ? WHERE id = ?", [newQty, id]);
    loadCart();
  };
  ...
  const handleCheckout = () => {
    if (cartItems.length === 0) {
      showToast("Add some products before checkout", "warning");
      return;
    }
  ...
  const performCheckout = () => {
    ...
    try {
      ...
      loadCart();
      showToast(
        `Order #${orderId} placed! Total: $${totalPrice.toFixed(2)} \ud83c\udf89`,
        "success",
      );
    } catch (error) {
